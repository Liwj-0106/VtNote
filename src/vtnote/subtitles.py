...
class SubtitleExportError(ValueError):
    """Raised when canonical text cannot be represented by a subtitle format."""
...
_ASS_HARD_SPACE_RE = re.compile(r"(?<!\\)\\h")
...
def _segment(index: int, start_ms: int, end_ms: int, text: str) -> TranscriptSegment:
    try:
...
def parse_ass(source: str) -> list[TranscriptSegment]:
    ...
    for raw_line in source.lstrip("\ufeff").replace("\r\n", "\n").replace("\r", "\n").splitlines():
        ...
        values = [
            value.strip()
            for value in line.split(":", 1)[1].split(",", len(event_format) - 1)
        ]
        ...
        text = _ASS_NEWLINE_RE.sub("\n", text)
        text = _ASS_HARD_SPACE_RE.sub(" ", text).strip()
...
def export_srt(segments: Iterable[TranscriptSegment]) -> str:
    ...
    if any(re.search(r"\n[ \t]*\n", segment.text) for segment in segments):
        raise SubtitleExportError("SRT text cannot contain an internal blank line")
    blocks = [
        f"{index}\n{_format_clock(segment.start_ms, ',')} --> "
        f"{_format_clock(segment.end_ms, ',')}\n{segment.text}"
...
def export_vtt(segments: Iterable[TranscriptSegment]) -> str:
    segments = tuple(segments)
    if any(re.search(r"\n[ \t]*\n", segment.text) for segment in segments):
        raise SubtitleExportError("WebVTT text cannot contain an internal blank line")
    blocks = [
...
def export_ass(segments: Iterable[TranscriptSegment]) -> str:
    ...
    events = []
    for segment in segments:
        if segment.end_ms // 10 <= segment.start_ms // 10:
            raise SubtitleExportError("ASS centisecond precision would create zero duration")
