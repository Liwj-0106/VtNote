...
| FR-003 | URL 校验与安全探测 | Task 2, 3, 6 | 部分实现 | `url_security.py`; `test_url_security.py`; API 注入探测和 redirect 报告重校验测试 | 真实网络 transport 的逐连接 DNS pin、受控 redirect、yt-dlp 网络边界；平台 corpus |
...
| FR-007 | 音频取得与 FFmpeg 预处理 | Task 3, 6 | 部分实现 | `media.py`; `test_media.py` 覆盖 shell-free、超时、真实 FFmpeg、部分文件隔离、原子发布 | 平台 audio handoff；worker 取消/重试；资源与磁盘故障 E2E |
...
| FR-009 | 火山极速版 Base64 ASR | Task 3, 7 | 部分实现 | 固定 resource/config 与云音频原语；`test_pipeline_contract.py::test_volc_resource_is_fixed_and_not_caller_configurable`; `test_real_ffmpeg_cloud_conversion...` | eligibility、HTTP、Base64 OGG/Opus、响应映射、2h/100MB 和应用阈值测试；live POC |
| FR-010 | faster-whisper 与云转本地 | Task 3, 7 | 部分实现 | 依赖精确 pin；D 盘模型默认合同；云授权路由测试；无生产 transcribe 调用 | lazy model、CPU/GPU、取消、provenance、明确失败 fallback、unknown outcome 不盲重试 |
...
| FR-013 | 可选 AI 笔记 | Task 2, 4, 5 | 部分实现 | template/profile/default/parallel stage 合同与 note typed path 存在 | chunk/map/reduce generator、prompt/response 上限、AI 标识、UI、事实质量 POC |
...
| FR-019 | 执行摘要与 provenance | Task 1, 2, 3, 4, 5 | 部分实现 | provenance schema、pipeline snapshot、stage attempts、sanitized public view 与基础确定性 Markdown 导出；现有 Markdown 未包含完整来源证明 | adapter/model/request/fallback/authorization 全链写入；JSON/Markdown 完整安全来源证明；详情呈现；无遥测 |
...
| NFR-005 | Windows/Python/CPU/GPU 兼容 | Task 3, 6, 7 | 部分实现 | Python 范围与精确依赖 pin；环境记录 native runtime；真实 FFmpeg 测试 | Win10/11 x64、CPU 与已列 GPU/CUDA 矩阵；安装/首次模型下载 E2E |
| NFR-006 | 键盘、AA、缩放、减弱动画 | Task 5, 7 | 未实现 | 当前无 React UI | 键盘核心旅程、焦点、屏幕阅读器、axe/人工 AA、200% 与 4 视口 |
| NFR-007 | 结构化脱敏日志与诊断包 | Task 2, 3, 6, 7 | 部分实现 | `sanitize_diagnostic` 写入边界与统一 API error shape 测试 | worker/launcher 结构化日志、轮转、关联 ID、诊断包、全局 secret/path scan |
...
| NFR-010 | 条款、版权、依赖许可与无绕过 | Task 3, 5, 6, 7 | 持续门禁 | URL allowlist、无 Cookie/plugin、参考项目许可审计 | UI 权利提示；实际 artifact SBOM/NOTICE；平台/供应商条款和来源访问日复核 |
...
| Task 3 | worker、来源、Volc/local ASR、回收 | 仅 3A runtime asset 与 3B ingress/media 原语完成；主体未完成 | lease/heartbeat、真实 adapter、逐连接网络边界、Volc/local、fallback、取消/重试 E2E |
| Task 4 | 翻译、笔记、编排 | 未完成 | AI failure 不重跑 source/transcribe；结构化产物/一次受控 retry |
| Task 5 | React 网站 | 未完成 | 三个顶层 surface、API client/component tests、静态 build |
| Task 6 | launcher、集成、安全、文档 | 未完成 | first-run、监督、日志、静态深链、安装/运行、安全回归 |
| Task 7 | 全项目 review/release | 未完成 | POC、全测试、依赖/许可/secret scan、构建/启动 smoke、未决项签字 |
...
| 云明确失败、auto 转本地 | FR-008–FR-010, FR-014 | 一次云调用、本地成功、fallback provenance |
...
|---|---|---|---|
| 30–50 视频平台/ASR POC | FR-003, FR-006, FR-009, FR-010; ADR-005/006/008 | 未运行 | corpus manifest、固定版本、原始度量、失败样本、签字结论 |
...
| chat 翻译/笔记质量 | FR-012, FR-013 | 未运行 | cue 完整、意义保持、事实覆盖/虚构率、人工双评 |
| Windows 性能矩阵 | NFR-005, NFR-008 | 仅依赖/FFmpeg 基础 | CPU/GPU、RTF、内存、磁盘、模型下载与驱动表 |
| 实际分发许可证 | NFR-010 | 参考依赖初审 | lock、wheel/binary/model SBOM、NOTICE、license bundle |
...
| THR-002 云应用阈值 | ASR + Backend + 产品 | 官方 2h/100MB 是 provider 上限；应用限制未冻结 | Base64 内存/时长 POC 后、Volc HTTP 前冻结 |
