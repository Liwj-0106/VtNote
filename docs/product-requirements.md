...
设置 ASR 模式 `auto|cloud|local`、翻译默认值和笔记模板。任务创建时必须保存不可变
配置快照，后续修改默认项不得改变已入队任务。
...
- 翻译默认关闭；笔记只有在首个当前修订配置档测试成功后才可一次性自动启用，
  用户关闭后不得再次自动开启；
...
需要 ASR 时，平台适配器只取得执行所需音频；本地媒体经 FFmpeg 校验和无 shell 参数
调用转换为受控音频资产。云端与本地可使用不同输出容器，但都必须原子发布。
...
上传 16 kHz、单声道 OGG/Opus，并解析分段/词信息为提供商无关结果。V1 不实现
`audio.url` 或云端切片。实现前必须重新核验
[官方文档](https://docs.volcengine.com/docs/6561/1631584?lang=zh)的端点、限制和
计费。

验收：

- 官方当前上限为 2 小时/100 MB；应用按批准计划采用更保守的预检/建议阈值，超过
  应用硬限制时在发送前拒绝，格式不符时只在本地重新转码；
- 请求 ID、HTTP 状态和安全错误码可审计，密钥与 Base64 音频不进日志；
- 明确的认证、配额、格式或服务错误可被分类；
- 连接中断或响应超时且服务端结果未知时标记 `cloud_outcome_unknown`，不得自动再次
  提交云请求。

### FR-010 本地 faster-whisper 与云端后备

本地转录使用已固定版本的 faster-whisper/CTranslate2，以分段级时间戳输出。`auto`
模式下，云端在明确未受理或明确失败后可自动转本地；结果不确定的云超时可转本地并
保留“云端可能已计费”警告，但不得重发云请求。`cloud` 模式失败后由用户明确选择
“改用本地”或修复后创建新的云端尝试。

验收：

- 首次需要模型下载时展示大小/来源/进度和磁盘错误；
- CPU 必须可运行，GPU 仅作为探测后的加速选项；
- 云转本地后只有一个规范转录被发布，云端迟到结果不得覆盖；
- 模型、设备、计算精度和运行耗时写入产物 provenance。
...
用户可选择“摘要”“关键点”或自定义模板及输出语言。笔记从不可变原文转录生成，
应携带任务/转录来源标识，并明确标注为 AI 生成内容。

验收：

- 自定义模板必须含非空提示词；
- 提示词和响应大小有上限，输入按可追踪片段分块；
- 笔记失败不阻止转录/译文交付；
- 可独立重试笔记，不重跑成功的平行翻译；
...
V1 支持 Windows 10/11 x64、Python 3.11、现代 Chromium/Edge 浏览器。CPU 是最低
可用路径；NVIDIA GPU 是可选加速。具体驱动/CUDA 组合须在发布矩阵中列明，未验证
组合不得标为支持。
...
日志采用结构化事件和稳定错误码，包含 task/item/stage/attempt 关联 ID、耗时和安全
诊断，不包含密钥、认证头、媒体字节、完整提示词或未清理本机路径。用户可导出脱敏
诊断包。
...
| THR-002 | 低于供应商 2 小时/100 MB 的云端应用建议/硬限制 | 未冻结（TBD） | ASR + Backend + 产品 | Task 3 火山 HTTP 实现前；依据 Base64 放大、峰值内存与 POC 时长分布 |
...
- 记录地区、访问状态、采样日期，不纳入登录/会员/DRM 内容。
...
| 云端成本 | 实际计费、每音频分钟成本、上传字节、请求次数、潜在重复计费 |
| 翻译 | 意义保持、专名/数字保持、cue 完整率、人工 1–5 分 |
| 笔记 | 关键事实覆盖率、可回溯陈述比例、虚构/错误陈述率、人工可用性评分 |

### 11.4 对照与决策

同一无字幕样本在条件允许时运行火山极速版与本地 faster-whisper；只在格式兼容且账户
同意时加入 OpenAI/AssemblyAI 作为研究对照，不进入 V1 实现范围。记录失败而不是剔除
...
在 POC 完成前，这六项均标记为“待验证”，不得写成已确认性能。
